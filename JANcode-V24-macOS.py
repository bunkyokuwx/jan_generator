...
class RoundedButton(tk.Label):
    def __init__(self, master, text, command, width=100, height=32, radius=8, 
                 ...
                 disabled_bg="#D1D5DB", parent_bg="#F8F9FA", font=("Helvetica", 10, "bold"), **kwargs):
...
class CustomCheckbutton(tk.Frame):
    def __init__(self, master, text, variable, command=None, bg="#F3F4F6", fg="#374151", size=22, **kwargs):
        ...
        self.lbl = tk.Label(self, text=text, bg=bg, fg=fg, font=("Helvetica", 10), cursor="hand2")
...
class JanInputRow:
    def __init__(self, parent, generator, remove_callback):
        ...
        font_header = ("Helvetica", 8)
        font_val = ("Helvetica", 12, "bold")
...
class FlatJANApp(tk.Tk):
    ...
    def _force_foreground(self):
        # macOS環境で起動時に裏に隠れるのを防ぐためのAppleScript制御
    ...
    def _build_ui(self):
        ...
        tk.Label(main_frame, text="①商品コードを入力", font=("Helvetica", 14, "bold"), bg="#F8F9FA", fg="#1F2937").pack(anchor="w")
        tk.Label(main_frame, text="商品コードを枠内に入力し、コードが自動生成します。", font=("Helvetica", 9), bg="#F8F9FA", fg="#4B5563").pack(anchor="w", pady=(2, 10))
        ...
        tk.Label(seq_frame, text="商品コード連番の一括作成", font=("Helvetica", 10, "bold"), bg="#F8F9FA", fg="#374151").pack(anchor="w", pady=(0, 5))
        
        input_area = tk.Frame(seq_frame, bg="#F8F9FA")
        input_area.pack(fill=tk.X)
        
        tk.Label(input_area, text="開始", font=("Helvetica", 10), bg="#F8F9FA").pack(side=tk.LEFT)
        self.entry_start = ttk.Entry(input_area, width=8, font=("Helvetica", 10))
        self.entry_start.pack(side=tk.LEFT, padx=5)
        
        tk.Label(input_area, text="〜終了", font=("Helvetica", 10), bg="#F8F9FA").pack(side=tk.LEFT)
        self.entry_end = ttk.Entry(input_area, width=8, font=("Helvetica", 10))
        ...
        tk.Label(main_frame, text="②出力設定", font=("Helvetica", 14, "bold"), bg="#F8F9FA", fg="#1F2937").pack(anchor="w", pady=(20, 5))
        ...
        RoundedButton(btn_frame, text="出力", command=self._export, width=280, height=44, radius=8, 
                      bg_color="#3B82F6", fg_color="#FFFFFF", hover_bg="#2563EB", font=("Helvetica", 14, "bold")).pack()
